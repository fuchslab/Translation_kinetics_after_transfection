...
#SBATCH --job-name=stan_sampling_ODE_model_based_on_solution_unknown_t0
...
# `sbatch bash_scripts/stan_sampling_ODE_model_based_on_solution_mult_error_unknown_t0_submit.sh <argument>`
...
if [ ! -f "intermediate_output_files/stanfit_objects/${1}_ODE/stanfit_object_$SLURM_ARRAY_TASK_ID.rds" ] ; then
    ...
    start_container ${WORK_DIR}/bash_scripts/stan_sampling_ODE_model_based_on_solution_unknown_t0_run.sh $1 $SLURM_ARRAY_TASK_ID

    # move and rename output and error message files
    mkdir -p "slurm_messages/$SLURM_JOB_NAME/${1}_ODE/"
    output_message_file="slurm_messages/$SLURM_JOB_NAME/${1}_ODE/output_$SLURM_ARRAY_TASK_ID.txt"
    error_message_file="slurm_messages/$SLURM_JOB_NAME/${1}_ODE/error_$SLURM_ARRAY_TASK_ID.txt"
