...
fig_format <- "pdf" # png pdf

dataset <- "simulated_data_dataset1_no_error"
obs_index <- 6
...
if(save_results){
  ODE_table <- print(xtable::xtable(stan_summary_ODE, digits = digits,
                     ...
                     booktabs = TRUE,
                     file = "figures_and_tables/tab_sim_data_no_error_ODE_summary.txt")
  ODE_table <- readLines("figures_and_tables/tab_sim_data_no_error_ODE_summary.txt")
  writeLines(ODE_table[5:(length(ODE_table)-1)], 
             con = "figures_and_tables/tab_sim_data_no_error_ODE_summary.txt" )
...
param = c("theta", "m0", "scale", "prod_theta2_m0", "prod_theta2_scale",
          "prod_m0_scale", "prod_theta2_m0_scale")
true_values_SDE <- c(pars$theta, pars$m0, pars$scale,
                     pars$theta[2]*pars$m0, pars$theta[2]*pars$scale, 
                     pars$scale*pars$m0, pars$theta[2]*pars$m0*pars$scale)
param_names <- c("$\\theta_1$","$\\theta_2$","$\\theta_3$", 
                                "$m_0$", "$\\mathrm{scale}$", "$\\theta_2m_0$", 
                                "$\\theta_2\\mathrm{scale}$", 
                                "$m_0\\mathrm{scale}$", 
                                "$\\theta_2m_0\\mathrm{scale}$")
...
if(save_results){
  SDE_table <- print(xtable::xtable(stan_summary_SDE, digits = digits,
                     ...
                     sanitize.text.function=function(x){x},
                     booktabs = TRUE,
                     file = "figures_and_tables/tab_sim_data_no_error_SDE_summary.txt")
  SDE_table <- readLines("figures_and_tables/tab_sim_data_no_error_SDE_summary.txt")
  writeLines(SDE_table[5:(length(SDE_table)-1)], 
             con = "figures_and_tables/tab_sim_data_no_error_SDE_summary.txt" )
}else{
...
if(save_results){
  if(fig_format == "pdf"){
    pdf("figures_and_tables/Fig_pairs_sim_data_no_error_theta_1_3.pdf", 
        width = 5, height = 4.4)
  }else if(fig_format == "png"){
    png("figures_and_tables/Fig_pairs_sim_data_no_error_theta_1_3.png", 
        width = 750, height = 660, pointsize = 24)
  }
}
...
pairs_of_2stanfits(SDE_stanfit_object, ODE_stanfit_object,
                   ...
                   name1 = expression(paste("Posterior sample for ", bold("SDE"), " model")),
                   name2 = expression(paste("Posterior sample for ", bold("ODE"), " model")))

if(save_results) dev.off()

if(save_results){
  if(fig_format == "pdf"){
    pdf("figures_and_tables/Fig_pairs_sim_data_no_error_theta_2_m0_scale.pdf", 
        width = 6.35, height = 5.6)
  }else if(fig_format == "png"){
    png("figures_and_tables/Fig_pairs_sim_data_no_error_theta_2_m0_scale.png", 
        width = 1260, height = 1130, pointsize = 32)
  }
}
...
if(save_results){
  if(fig_format == "pdf"){
    pdf("figures_and_tables/Fig_pairs_sim_data_no_error_theta_2_m0_scale_all.pdf", 
        width = 10, height = 9.4)
  }else if(fig_format == "png"){
    png("figures_and_tables/Fig_pairs_sim_data_no_error_theta_2_m0_scale_all.png", 
        width = 2000, height = 1880, pointsize = 32)
  }
}
param <- c("theta[2]", "m0", "scale",
...
if(save_results){
  file_name <- "figures_and_tables/tab_sim_data_no_error_CI_lengths.txt"
...
if(save_results){
  pdf("figures_and_tables/Fig_aggregated_sampling_output_sim_data_no_error_median_of_CI_length_normalized_by_true_value.pdf", 
...
if(save_results){
  pdf("figures_and_tables/Fig_aggregated_sampling_output_sim_data_no_error_median_of_CI_length.pdf", 
      width = 5.7, height = 4.4)
}
plot_aggregate_values(arr_summary_1 = arr_summary_ODE, 
                      arr_summary_2 = arr_summary_SDE,
...
if(save_results){
  ggsave("figures_and_tables/Fig_traceplots_sim_data_no_error.pdf", plot = p,
         width = 8, height = 8)
}else{
  p
}